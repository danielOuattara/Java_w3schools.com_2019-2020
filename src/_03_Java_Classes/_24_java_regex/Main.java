package _03_Java_Classes._24_java_regex;
...
public class Main {
    ...
    a search pattern. When you search for data in a text, you
    can use this search pattern to describe what you are searching
    for.
    ...
    can import the java.util.regex package to work with regular
    expressions.
    ...
    Example: Find out if there are any occurrences of the word
    ...
    In this example, the word "w3schools" is being searched
    for in a sentence:
    > The 'pattern' is created using the 'Pattern.compile()'
    method. The first parameter indicates which pattern is
    being searched for and the second parameter has a flag to
    indicates that the search should be case-insensitive. This
    second parameter is optional.

    > The 'matcher()' method is used to search for the pattern
    in a string. It returns a Matcher object which contains
    information about where the search that was performed.
    ...
    Flags
    --------
    Flags in the compile() method change how the search is
    performed. Here are a few of them:
    ---------------------------------------------------------------------------------------------
    ...
    > Pattern.UNICODE_CASE - Use it together with the CASE_INSENSITIVE flag to also ignore the case
                             of letters outside the English alphabet
    ---------------------------------------------------------------------------------------------
